...
export function HomePage() {
  ...
  const [filters, setFilters] = useState({
    collegeId: location.state?.collegeId || "",
    radius: 2,
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      {!filters.collegeId && (
        <div className="mt-5 rounded-3xl p-12 text-center" style={{ background: "var(--surface-container-low)" }}>
          ...
          <p className="mt-4 text-lg font-black" style={{ color: "var(--on-surface)" }}>Select your college to see listings</p>
          <p className="mt-2 text-sm" style={{ color: "var(--outline)" }}>
            Choose a city and campus in the filters above, then click <strong>Apply Filters</strong>.
          </p>
        </div>
      )}
      {filters.collegeId && query.isLoading && (
        <div className="mt-5 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-80 skeleton-shimmer" />)}
        </div>
      )}
      {filters.collegeId && query.isError && (
      ...
      {filters.collegeId && !query.isLoading && !query.isError && (query.data?.items.length ?? 0) === 0 && (
        <div className="mt-5 rounded-3xl p-12 text-center" style={{ background: "var(--surface-container-low)" }}>
          <p className="text-5xl">🔍</p>
